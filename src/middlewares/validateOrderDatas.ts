...
export const validateOrderDatas = [
  check('userId').optional().isInt().withMessage('userId must be an integer'),
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!errors.isEmpty()) {
      ...
      if (req.body.userId === undefined) {
        ...
        if (
          ![UserRole.MANAGER, UserRole.ADMIN].includes(res.locals.user.role)
        ) {
          next(
            new ResponseError(
              403,
              'Forbidden order',
              'Only managers and admins can create an order for another user',
            ),
          );
        }
      }
